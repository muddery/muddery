...
`quest` is the key of the quest that have this objective.<br>
`dependency` is this quest's dependency.<br>
...
Sentences are starts and ends of quests. Sentence can provide and/or finish quests.<br>
...
`dialogue` is the key of a dialogue.<br>
`default` is whether this dialogue is default or not. `0` means `NO` and `1` means `YES`. If an NPC has other dialogues, it will not show default dialogues. Default dialogues only show when other dialogues can not show.<br>
...
It is the data of how events trigger dialogues.<br>
`key` is the key of the event.<br>
`dialogue` is the key of the dialogue that will be triggered.<br>
`npc` is the key of an NPC. If it is not empty, the NPC speaker's name will be the NPC's name. If it is empty, the NPC speaker's name will be empty.<br>
