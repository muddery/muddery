...
class CmdQuit(Command):
    ...
    def func(self):
        ...
        if nsess == 2:
            player.msg({"msg":"{RQuitting{n. One session is still connected."}, sessid=self.sessid)
        elif nsess > 2:
            player.msg({"msg":"{RQuitting{n. %i session are still connected." % (nsess-1)}, sessid=self.sessid)
        else:
            # we are quitting the last available session
            player.msg({"msg":"{RQuitting{n. Hope to see you again, soon."}, sessid=self.sessid)
        player.msg({"logout":""})
